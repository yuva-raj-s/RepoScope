...
async function analyzeWithGemini(input: AnalysisInput, apiKey: string): Promise<AIAnalysis> {
  try {
...
async function analyzeWithOpenAI(input: AnalysisInput, apiKey: string): Promise<AIAnalysis> {
  try {
...
export async function analyzeRepository(
  ...
  if (provider === "gemini") {
    ...
    throw new Error(`Provider ${provider} not yet supported on client-side`);
  }
}
